...
function initBtnEvent(){
	...
	$("#btn_del").bind('click',function(){
		...
		layer.confirm("您确定要删除所选信息吗?",{
	        ...
	        var url=getRootPath()+"carTypePrice/delete";
			$.post(url,
					{id:selections+""},
    		        function(data){
						if(data.status=="1"){
...
var TableInit = function (){
	...
	oTableInit.Init = function (){
		$('#materialsTableInfo').bootstrapTable({
			url: getRootPath()+"carTypePrice/selectList",         //请求后台的URL（*）
...
function openSaveButton(type){
	...
	if(flag){
    	...
    	$.ajax({
            type: "post",
            url: getRootPath()+"carTypePrice/save",
            data: addJson,
            dataType: "json",
    		async : true,
...
function getDataForm(){
	...
	var carStandard = $("#carStandard").val();
	if (null == carStandard || "" == carStandard) {
		layer.msg("请输入车型描述", {
			time : 3000
		});
		return;
...
function editOrCheck(obj,type){
	...
	if(type==1){
		...
		$("#carType").attr("disabled",true);
   		$("#carStandard").attr("disabled",true);
   		$("#deposit").attr("disabled",true);
   		$("#carPrice").attr("disabled",true);
	}else{
		$('#btEmpAdd').show();
		$("#myModalTitle").html("编辑");
		$("#carType").attr("disabled",false);
   		$("#carStandard").attr("disabled",false);
   		$("#deposit").attr("disabled",false);
   		$("#carPrice").attr("disabled",false);
	}
	$("#id").val(obj.id);
	$("#carType").val(obj.carType);
	$("#carStandard").val(obj.carStandard);
	$("#deposit").val(obj.deposit);
	$("#carPrice").val(obj.carPrice);
